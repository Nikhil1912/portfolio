...
- **Image Processing:** sharp (build-time script via `tsx`) — `npm run photos:process`
- **Image Storage:** Cloudflare R2 (Phase 8; local `public/photos/` for dev)
- **Hosting:** Vercel
...
```
scripts/
└── process-photos.ts           # Image pipeline: reads originals/, generates WebP variants + blur
                                # placeholders, writes public/photos.json. Run: npm run photos:process.
                                # Single file by design — split into lib/ if it exceeds ~300 lines.

public/
└── photos.json                 # Canonical photo manifest (id, title, category, width, height,
                                # src, alt, order, placeholder, srcset). Generated by the pipeline;
                                # manual edits to title/category/alt/order are preserved on re-runs.
...
│   ├── gallery/
│   │   └── page.tsx            # Reads public/photos.json, passes photos prop to GalleryPage
│   └── contact/
...
│   │   └── index.ts
│   ├── gallery/
│   │   ├── components/         # PhotoGrid, LoadMoreButton, GalleryLightbox, GalleryPage
│   │   ├── hooks/              # usePhotoLoader
│   │   └── index.ts
│   └── contact/
...
├── types/                      # Shared TypeScript types and interfaces (includes Photo, PhotoSrcSet)
...
### Images
- All photos served via the optimized pipeline (`npm run photos:process`). See DESIGN.md Section 7.
- Pipeline input: originals in `originals/` (gitignored). Supported formats: JPEG, PNG, WebP, TIFF, HEIF, HEIC.
- Pipeline output: four WebP sizes per photo (thumbnail 400px, medium 800px, large 1600px, full 2400px) in `public/photos/[id]/` (gitignored), plus a base64 blur placeholder inlined in `public/photos.json`.
- EXIF orientation is always applied before resizing — portrait photos from phones will be correctly oriented.
- `public/photos.json` is the canonical manifest. Edit `title`, `category`, `alt`, `order` there after running the pipeline; those edits survive re-runs.
- Grid images are lazy-loaded. First row loads eagerly.
- `PhotoGrid` passes `srcSet` + `sizes` to `<img>` for responsive delivery; `GalleryLightbox` uses `srcset.full`.
